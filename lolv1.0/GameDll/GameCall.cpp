...
void __stdcall SkillHookStub(DWORD skillObj, PFLOAT xyz, PDWORD monsObj)
{
	if (g_MonsterObj) {
		...
		EM_POINT_3D pnt;
		pnt.x = temp.GetPoint().x + temp.GetMonsterOrientation().x * (float)(10.0);
		pnt.z = temp.GetPoint().z + temp.GetMonsterOrientation().z * (float)(10.0);
		pnt.y = temp.GetPoint().y + temp.GetMonsterOrientation().y * (float)(10.0);
